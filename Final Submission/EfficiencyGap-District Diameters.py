...
# Distance between towns in a district less than 120 km for 3 districts, 90 km for other 6 districts
# 120 >= dist_st * (z_s^i + z_t^i - 2) for all districts i, towns s<t
for i in range(3):
    for s in range(len(TOWNS)):
        for t in range(s+1,len(TOWNS)):
            m.addConstr(110 - DIST[s][t] * (z[s][i] + z[t][i] - 1), sense=gb.GRB.GREATER_EQUAL, rhs=0., name="r"+str(i)+'_'+str(s)+'-'+str(t))

for i in range(3,7):
    for s in range(len(TOWNS)):
        for t in range(s+1,len(TOWNS)):
            m.addConstr(70 - DIST[s][t] * (z[s][i] + z[t][i] - 1), sense=gb.GRB.GREATER_EQUAL, rhs=0., name="r"+str(i)+'_'+str(s)+'-'+str(t))            

for i in range(7,NUMDISTRICTS):
    for s in range(len(TOWNS)):
        for t in range(s+1,len(TOWNS)):
            m.addConstr(40 - DIST[s][t] * (z[s][i] + z[t][i] - 1), sense=gb.GRB.GREATER_EQUAL, rhs=0., name="r"+str(i)+'_'+str(s)+'-'+str(t))            
...
m.setParam('TimeLimit', 10*60) # Stop after 10 minutes
print('SOLVING')
m.optimize()
...
print(0.0001*sum([ri.x for ri in r]))
sum([ri.x for ri in r])
print(sum([ri.x for ri in Ddiff]))
print([ri.x for ri in Ddiff])
print(y/VOTERS)
...
#NOTE: MAY NEED TO RELAX POPULATION CONSTRAINTS
