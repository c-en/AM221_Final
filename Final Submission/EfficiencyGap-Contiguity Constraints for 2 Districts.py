...
ftwo = [[] for t in range(NUMTOWNS)]
for t, ft2 in enumerate(ftwo):
    for i in range(NUMTOWNS):
        ft2.append(m.addVar(name='ftwo_'+str(t)+'('+str(i)+')'))
...
sinktwo = []
for i in range(NUMTOWNS):
    sinktwo.append(m.addVar(name='sinktwo'+str(i), vtype=gb.GRB.BINARY))
...
m.addConstr(sum(sinktwo), sense=gb.GRB.EQUAL, rhs=1, name='onesinktwo')

for t in range(NUMTOWNS):
    m.addConstr(sink[t]-z[t][0], sense=gb.GRB.LESS_EQUAL, rhs=0, name='sinkdistrict'+str(t))
    m.addConstr(sinktwo[t]-z[t][1], sense=gb.GRB.LESS_EQUAL, rhs=0, name='sinktwodistrict'+str(t))
...
for t in range(NUMTOWNS):
    ...
    m.addConstr(sum([ftwo[t][t2] for t2 in np.where(ADJ[t]==1)[0]])+(1-NUMTOWNS)*z[t][1], sense=gb.GRB.LESS_EQUAL, rhs=0., name='flowtwo'+str(t))
...
for t in range(NUMTOWNS):
    ...
    m.addConstr(sum([ftwo[t][t2] for t2 in np.where(ADJ[t]==1)[0]])-sum([ftwo[t3][t] for t3 in np.where(ADJ[t]==1)[0]])-z[t][1]+NUMTOWNS*sinktwo[t], sense=gb.GRB.GREATER_EQUAL, rhs=0., name='flowconstrainttwo'+str(t))
...
print('SOLVING')
m.setParam('Heuristics', 0.10)
m.setParam('TimeLimit', 5*60) # Stop after 5 minutes
...
# Check only using one set of constraints
